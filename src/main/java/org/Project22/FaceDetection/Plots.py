...
output_data_on_dataset = {'OpenCV': {'average_iou': 0.21356696150864374, 'mean_average_precision': 0.16094956345924968, 'average_inference_time': 49.498165828872745},
        ...
        'DlibHOGFaceDetector': {'average_iou': 0.2530937465890223, 'mean_average_precision': 0.12089938464330218, 'average_inference_time': 141.02376511738932}}
...
def plot_iou_vs_time_complexity(data):
    ...
    plt.title('Intersection over Union (IoU) Ratio with Respect to Time Complexity', fontsize=16, fontweight='bold')
...
def plot_running_time(output_data_running_time):
    ...
    plt.bar(methods, running_times, width=0.5, color=colors)
...
def plot_webcam_experiment(detected_not_detected):
    ...
    plt.bar(methods, detected_face_percent, width=0.5, color=colors)
    plt.xlabel('Methods', fontweight='bold', fontsize=14)  
